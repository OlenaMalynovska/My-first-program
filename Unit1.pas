...
var f:file of gromadjany;           //файл записів
    z:gromadjany;                   //запис
    namefile:string;                //ім'я файла
    nbtn2:integer;
          
{Встановлення початкових властивостей компонентів та забезпечення їх доступу на формі}
...
begin
 if Edit6.Text='' then
  begin
   ShowMessage('Ви не ввели ім я файла!'+#13+'Повторіть введення!');
   Exit;
  end;
 namefile:=Edit6.Text;     //Введення імені файла
...
for i:=0 to ComponentCount-1 do
 begin
  ...
  //Всі компоненти Edit видимі  активізовані
  if components[i] is TEdit then (components[i] as TEdit).Visible:=true;
 end;

{Закладка ВВЕДЕННЯ ДАНИХ}

Edit6.Clear;Edit6.Hide;        //Edit6 очищений і прихований
Label10.Hide;Button2.Hide;     //Компоненти приховані
GroupBox1.Visible:=true;
GroupBox1.Enabled:=true;
Edit1.SetFocus;                //Компонент містить курсор введення
Edit2.Enabled:=false;          //Компоненти неактивізовані
Edit3.Enabled:=false;
Edit4.Enabled:=false;
Edit5.Enabled:=false;
DateTimePicker1.Visible:=true;  //Компонент видимий
DateTimePicker1.Enabled:=false; //Компоненти неактивізовані
ComboBox1.Enabled:=false;
ComboBox1.Visible:=true;        //Компоненти видимі
Button1.Visible:=true;
Button2.Enabled:=false;         //Компонент неактивізований
...
{Закладка ЗАПИТ1}
GroupBox2.Visible:=true;         //Компонент видимий
Edit8.Enabled:=false;            //Компонент неактивізований
Edit9.Enabled:=false;            //Компонент неактивізований
Button5.Visible:=true;           //Кнопка видима
Button5.Enabled:=false;          //Кнопка неактивізована
StringGrid2.Hide;
Button6.Visible:=false;          //Кнопка невидима
 {Закладка ЗАПИТ2}
Button7.Visible:=true;           //Кнопка видима
Button7.Enabled:=false;          //Кнопка неактивізована
GroupBox3.Visible:=true;         //Компоненти видимі
DateTimePicker2.Visible:=true;
DateTimePicker3.Visible:=true;
DateTimePicker3.Enabled:=false;  //Компонент неактивізований
Label18.Visible:=false;          //Компоненти невидимі
Label19.Visible:=false;

{Закладка Запит 3}
Button9.Visible:=true;           //Кнопка видима
StringGrid4.Enabled:=false;      //Компонент неактивізований
Label21.Visible:=false;          //Компоненти невидимі
...
//Активізація компонента Edit4
procedure TForm1.Edit3Chane(Sender: TObject);
begin
 Edit4.Enabled:=true;
end;
//Активізація компонента Edit5
procedure TForm1.Edit4Change(Sender: TObject);
begin
 Edit5.Enabled:=true;
end;
//Активізація компонента ComboBox1
procedure TForm1.Edit5Change(Sender: TObject);
begin
 ComboBox1.Enabled:=true;
end;
//Активізація компонента DateTimePicker1
procedure TForm1.ComboBox1Change(Sender: TObject);
begin
 DateTimePicker1.Enabled:=true;
end;
//Активізація кнопки Button1 "Додати"
...
begin
 ...
 if(length(Edit1.Text)=0)or(length(Edit2.Text)=0)or
   ...
   (length(ComboBox1.Text)=0)then
    begin
     ShowMessage('Ви не ввели дані про людину!');
     exit;
    end;
 {записування у файл значень про особу}
 ...
 begin
  ...
  with StringGrid1 do               //Підпис стовпців
   begin
    cells[0,0]:='Вулиця';
    cells[1,0]:='Будинок';
    cells[2,0]:='Номер квартири';
    cells[3,0]:='Площа';
    cells[4,0]:='ПІБ';
    cells[5,0]:='Кількість проживаючих';
    cells[6,0]:='Дата переселення';
    Visible:=false;
   end;
  StringGrid1.Show;
  Button4.Enabled;
...
//Підпрограма активізації Edit8
procedure TForm1.Edit7Change(Sender: TObject);
begin
 Edit8.Enabled:=true;
end;

//Підпрограма активізації Edit9
 ...
 begin
  ...
  with StringGrid2 do
   begin
    ...
    cells[2,0]:='Номер квартири';
    cells[3,0]:='ПІБ';
    cells[4,0]:='Кількість проживаючих';
    Visible:=false;
   end;
  StringGrid2.Show;
  Button4.Enabled;
...
//Підпрограма активізації DateTimePicker3
...
begin
 ...
 with StringGrid3 do
  begin
   cells[0,0]:='Дата переїзду';
   cells[1,0]:='ПІБ власника';
   cells[2,0]:='Адреса';
   cells[3,0]:='Кількість осіб';
   Visible:=false;
  end;
 StringGrid3.Show;
 Button8.Enabled;
   ...
   with z do
    if (datap>=d1) and (datap<=d2) then
     begin
      ...
      StringGrid3.Cells[2,i]:=vul+', б.'+IntToStr(bud)+', кв.'+IntToStr(nkv);
...
begin
 reset(f);                         //Відкрити файл
 seek(f,0);                        //Перенести покажчик на початок файла
 //Підпис стовпців компонента StringGrid4
 with StringGrid4 do
 begin
  cells[0,0]:='Адреса';
  cells[1,0]:='Площа';
  cells[2,0]:='Кількість проживаючих';
  cells[3,0]:='Площа на 1 особу';
  Visible:=false;
 end;
 //Заповнення таблиці з розрахунком площі
 i:=0;
while not eof(f) do
 begin
  read(f,z);
  with z do
   begin
    ...
    StringGrid4.Cells[0,i]:=vul+', б.'+IntToStr(bud)+', кв.'+IntToStr(nkv);
 ...
 mins:=100;
 maxs:=0;
 //Пошук мінімальної та максимальної площі на особу
 seek(f,0);
 while not eof(f) do
  begin
   read(f,z);
  ...
  begin
   ...
   with z do
    begin
     if  pl/kpr=maxs then
       begin
        s:=vul+', б.'+IntToStr(bud)+', кв.'+IntToStr(nkv);
        Memo1.Lines.Add(s);
       end;
     if  pl/kpr=mins then
       begin
...
procedure TForm1.Button10Click(Sender: TObject);
 var i,j: integer;
begin
 Memo1.Clear;                             //очистити Memo1
 Memo2.Clear;                             //очистити Memo2
 Memo1.Visible:=false;                    //компонент невидимий
 Memo2.Visible:=false;                    //компонент невидимий
 for i:=0 to StringGrid4.RowCount-1 do          //очистити StringGrid4
 for j:=0 to 4 do StringGrid4.Cells[j,i]:='';
 Label21.Visible:=false;                   //компонент невидимий
 Label22.Visible:=false;                   //компонент невидимий
 Label23.Visible:=false;                   //компонент невидимий
 Label24.Visible:=false;                   //компонент невидимий
 StringGrid4.Visible:=false;               //компонент невидимий
 Button10.Visible:=false;                  //компонент невидимий
 Button9.Enabled:=true;


end;
//Підпрограма команди меню Про автора
procedure TForm1.N3Click(Sender: TObject);
begin
 ShowMessage('Автор:студентка групи ІДП-32 Малиновська Олена Олександрівна')
end;
